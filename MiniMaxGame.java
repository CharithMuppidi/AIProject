...
public class MiniMaxGame {
	...
	public MiniMaxGame(String currentBoard, int depth) 
	{
	...
	private int staticEstimate(char[] b) {
		...
		numBlackMoves = generateBlackMoves(b).size();
		if (numBlackPieces <= 2)
			return 10000;
	...
	public static void main(String[] args) {
		...
		try {
			...
			try (Scanner scan = new Scanner(fish)) {
				...
				MiniMaxGame ed = new MiniMaxGame(board, depth);
				String bob = new String(ed.newBoard);
				System.out.println("Board Position: " + bob);
				System.out.println("Positions evaluated by static estimation: " + ed.pos_eval);
				System.out.println("MINIMAX estimate: " + ed.stat_est);
				otter.write(bob);
			}
			fish.close();
			otter.close();
