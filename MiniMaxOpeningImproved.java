...
public class MiniMaxOpeningImproved {
	...
	public MiniMaxOpeningImproved(String currentBoard, int depth) 
	{
	...
	private int staticEstimate(char[] b) {
		...
		int numWhiteMoves = generateMovesOpening(b).size();
		int numBlackMoves = generateBlackMoves(b).size();
		int result = (numWhitePieces + numWhiteClosedMills + numWhiteMoves) - (numBlackPieces + numBlackClosedMills + numBlackMoves);
		return result;
	}
	...
	public static void main(String[] args) {
		...
		try {
			...
			try (Scanner scan = new Scanner(fish)) {
				...
				MiniMaxOpeningImproved betterStart = new MiniMaxOpeningImproved(board, depth);
				String bob = new String(betterStart.newBoard);
				System.out.println("Board Position: " + bob);
				System.out.println("Positions evaluated by static estimation: " + betterStart.pos_eval);
				System.out.println("MINIMAX estimate: " + betterStart.stat_est);
				otter.write(bob);
			}
			fish.close();
			otter.close();
